...
public class LaunchPad : PhysicsEntity
{
    ...
    private readonly B2ShapeId shape;
    private readonly B2ContactData[] contacts;

    private const float displacementAmount = 5f;
    ...
    public override void Update()
    {
        ...
        if (moving)
        {
            ...
            Vector2 target;
            if (forward)
                target = -Vector2.UnitY * Ease.Cube.In(currentTime / displacementTime) * displacementAmount + originalPos;
            else
                target = -Vector2.UnitY * (1 - Ease.Cube.In(currentTime / (displacementTime * 3))) * displacementAmount + originalPos;
            ...
            if ((forward && currentTime > displacementTime) || (!forward && currentTime > 3 * displacementTime))
            {
